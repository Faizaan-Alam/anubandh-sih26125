  ...
  return (
    <div className="space-y-4 max-w-5xl">
      <PageHeader kicker="Asset NFT" title={`Asset #${tokenId}`}>
        <Link className="text-primary font-semibold" href="/assets">Back to registry</Link>
        {" · "}
      ...
      {asset && (
        <div className="grid md:grid-cols-2 gap-4">
          <div className="card bg-base-100 shadow-md p-4 space-y-3 text-sm fade-in-up">
            ...
            <div className="flex justify-between gap-2 items-center">
              <span className="opacity-80">Quarantine</span>
              <StatusBadge value={asset.quarantined ? "Quarantined" : "Clear"} />
            </div>
            <div className="flex justify-between gap-2">
              <span className="opacity-80">High-value</span>
              <span>{String(asset.highValue)}</span>
            </div>
            <div>
              <div className="opacity-80">Metadata hash</div>
              <div className="font-mono text-xs break-all mt-1">{String(asset.metadataHash)}</div>
              <div className="text-xs opacity-60 mt-1">The raw document is off-chain. Only this hash is on-chain.</div>
            </div>
          </div>
          <div className="card bg-base-100 shadow-md p-4 space-y-3 text-sm fade-in-up" style={{ animationDelay: "50ms" }}>
            ...
            {breakdown?.explanation != null && (
              <p className="text-xs opacity-80 leading-relaxed">{String(breakdown.explanation)}</p>
            )}
      ...
      <div className="card bg-base-100 shadow-md p-4 space-y-3">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div className="space-y-2">
            <label className="text-xs font-semibold opacity-80">New owner</label>
            <select className="select select-bordered w-full" value={newOwner} onChange={(e) => setNewOwner(e.target.value)}>
              {DEMO_ACCOUNTS.map((a) => (
          ...
          <div className="space-y-2">
            <label className="text-xs font-semibold opacity-80">New custodian</label>
            <select className="select select-bordered w-full" value={newCustodian} onChange={(e) => setNewCustodian(e.target.value)}>
              {DEMO_ACCOUNTS.map((a) => (
                <option key={a.address} value={a.address}>{a.label}</option>
              ))}
            </select>
            <button className="btn btn-outline" onClick={allocate}>Allocate custody</button>
          </div>
        </div>
